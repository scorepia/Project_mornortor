    ...
    with tf.compat.v1.Session(config=config) as sess:
        ...
        while True:
            ...
            if args.info:
                ...
                title_name = args.text + " (style = " + str(args.style) + ", bias = " + str(args.bias) + ")"
                ...
                # Getting only the axes specified by ax[0,1]
                extent = ax[0,1].get_window_extent().transformed(fig.dpi_scale_trans.inverted())
                
                # Saving it 
                myls = args.save.split(".")
                mylspath = myls[0:-1]
                myext = "." + myls[-1]
                save_filename = ''.join(mylspath) + "-style-" + str(args.style) + "-bias-" + str(args.bias) + myext
                fig.savefig(save_filename, bbox_inches=extent.expanded(1.3, 1.3))
